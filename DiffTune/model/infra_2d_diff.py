...
class RelativeMultiHeadAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        B, S, _ = x.shape
        # 1) project to Q, K, V
        q = self.q_proj(x).view(B, S, self.num_heads, self.d_k).transpose(1,2)  # [B, H, S, d_k]
        ...
        N = self.rel_idx.size(0)                                   # e.g. 78
        # rel_bias[self.rel_idx] gives [N, N, H]
        bias_state = self.rel_bias[self.rel_idx]                   # [N, N, H]
        bias_state = bias_state.permute(2,0,1).unsqueeze(0)        # [1, H, N, N]


        # basically for long token inputs where we are putting in the horizon tokens we still keep pretrained 2d biases, but zero out for the future states/actions
        if S != N:
            full_bias = scores.new_zeros(1, self.num_heads, S, S)
            full_bias[..., :N, :N] = bias_state
            bias = full_bias
        else:
            bias = bias_state
...
class TransformerDecoder2D(nn.Module):
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs
        ...
        return {"inputs_embeds": inputs_embeds or self.input_emb(input_ids), **kwargs}
